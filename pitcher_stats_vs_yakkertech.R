...
yakkertech <- readRDS("~/Projects/yakkertech_data/total.csv")
...
yakkertech_data <- yakkertech %>% 
  ...
  mutate(hr = ifelse(hr == "12","0",hr),
         ...
         PitchType = case_when(RelSpeed < top_speed - 10 ~ "Offspeed",
                                     between(adj_SpinAxis, 45, 135) ~ "Screwball",
                                     between(adj_SpinAxis, 135, 225) ~ "Dropball",
                                     between(adj_SpinAxis, 225, 305) ~ "Curveball",
                                     TRUE ~ "Riseball"))
...
screw_model <- lm(wFIP ~ ., pitch_stats %>% filter(PitchType == "Screwball") %>% select(c(4,7,8,10,11,12,3)))
drop_model <- lm(wFIP ~ ., pitch_stats %>% filter(PitchType == "Dropball") %>% select(c(4,7,8,10,11,12,3)))
rise_model <- lm(wFIP ~ ., pitch_stats %>% filter(PitchType == "Riseball") %>% select(c(4,7,8,10,11,12,3)))
curve_model <- lm(wFIP ~ ., pitch_stats %>% filter(PitchType == "Curveball") %>% select(c(4,7,8,10,11,12,3)))
...
write_csv(combined_data, "~/Projects/softball-projects/combined_pitcher_data.csv")
